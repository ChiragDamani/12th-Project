...
def forget_password():
    ...
    if mailentry.get() == '':
        ...
        try:
            con = pymysql.connect(host='localhost', user='root', password='Lambrghini99', database='login_register')
            cur = con.cursor()
            cur.execute('select * from register where email=%s', mailentry.get())
            row = cur.fetchone()
            if row == None:
...
def signin():
    if mailentry.get() == '' or passentry.get() == '':
        ...
        try:
            con = pymysql.connect(host='localhost', user='root', password='Lamborghini99', database='login_register')
            cur = con.cursor()
            cur.execute('select * from register where email=%s and password=%s', (mailentry.get(), passentry.get()))
            row = cur.fetchone()
            if row == None:

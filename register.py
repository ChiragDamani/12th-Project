...
def register():
    if entryfirstname.get() == '' or entrylastname.get() == '' or entryemail.get() == '' or entrycontact.get() == '' or \
        ...
        try:
            con = pymysql.connect(host='localhost', user='root', password='Lamborghini99', database='login_register')
            cur = con.cursor()
            cur.execute('select * from register where email=%s', entryemail.get())
            row = cur.fetchone()
            if row != None:
                ...
                cur.execute(
                    'insert into register (f_name,l_name,email,contact,question,answer,password) values(%s,%s,%s,%s,%s,%s,%s)',
...
check = IntVar()
checkButton = Checkbutton(registerFrame, text='I Agree With All The Terms & Conditions', variable=check, onvalue=1,
